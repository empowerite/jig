...
ships. A policy declares types: what an instance is, where it lives, what it may be called, what it contains, how it
moves, and who may move it. jig reads every instance from the system that holds it, computes the next legal move from
the policy in force and the facts it observed, and drives that move through the system's own interface. The work item
and the change are the two types the standard policy declares: a work item is filed, assigned and resolved, and a
change is drafted, proposed, admissible, integrating, integrated and realized. A repository declares types of its own
the same way and refines the types declared above it, and a link above it, a team's or an organization's, declares for
every repository under it. jig owns none of the state it moves.
...
5. A judgment carries its evidence. Every verdict names the gate, the subject at a digest, the policy in force and who
   produced it, and is signed, so that anyone the repository answers to can verify it without jig.

### The parts

Each part has an RFC, and once accepted the RFC is the spec its code is checked against.

- Types, [jig-001-types.md](jig-001-types.md). A type is the unit of policy and declares six things: an identity, a
...
- jig is an engine over declared types rather than a control plane for one process because a lifecycle fixed in the
  binary put every organization on jig's release cadence and could not give a platform component a lifecycle an
  application service does not have, and because the engine that moves a change moves an rfc, a release or a
  deployment with nothing added but a declaration. The SDLC is the standard policy because it is the one every
  repository needs and the one the ports are proven against first.
...
- Orchestration is a function call because a function is tested once and trusted everywhere, answers a person, a
  session and a bot alike, and needs no model. Judgment stays with a named actor, so what is delegated is explicit and
  what is computed is total.
...
- A judgment carries its evidence because an audit asks what was in force and who said so, and a green check says only
  ...
  a record anyone can verify; a record only jig could verify would be one nobody outside jig could trust.
- The boundaries are drawn where a system already exists. A repository has a tracker, a host, a CI system and a
  ...
  one thing none of them has, the declared policy and the move computed from it.
